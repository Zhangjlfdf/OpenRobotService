...
- Service 逻辑（`info_node_ledger_sync_service.build_sync_preview`，与 5.8 共用 `info_node_import_service` 的匹配内核）：
  ...
  4. 再走内核的标题判等（归一后精确 + 相似度 ≥0.9 兜底）：台账列名要么对得上某个可填节点标题，要么不算匹配——台账是甲方口径的宽表，列名与节点名不同义的一律不猜（用户口径「匹配不到的先忽略」）。**这里没有大模型参与**，也没有 5.8 的「建议归属附近的下拉」兜底（台账条目不带 `suggested_parent_path`，`_vicinity_dropdowns` 对空路径返回空）；
