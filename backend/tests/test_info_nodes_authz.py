...
口径（与 app/modules/admin/api/info_nodes.py 顶部注释同一份）：
  结构类写接口  POST /projects/{id}（增补节点）、/projects/{id}/import（整树导入）、
                ...
                /projects/{id}/clear-values（一键清空已填值）、
...
def svc(monkeypatch):
    ...
    mocks = SimpleNamespace(
        ...
        clear_values=MagicMock(return_value={"cleared": 3}),
...
def test_project_member_can_add_node_and_import_tree(client, svc):
    ...
    cleared = client.post("/api/admin/info-nodes/projects/P1/clear-values")   # 一次抹掉全项目已填的值
...
def test_non_member_is_denied_on_every_structural_route(client, svc):
    ...
    for response in (
        ...
        client.post("/api/admin/info-nodes/projects/P2/clear-values"),
    ...
    svc.clear_values.assert_not_called()
