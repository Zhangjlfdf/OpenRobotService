...
```
                ...
                └───────────────────┬─────────────────────┘
                                    │ HTTPS
                ┌───────────────────▼─────────────────────┐
                │      前端 H5（React 19 + TS + TDesign Mobile）      │
                │  我要摇人 · 系统任务 · 后台管理（角色化）   │
                └───────────────────┬─────────────────────┘
                                    │ REST / SSE(流式)
                ┌───────────────────▼─────────────────────┐
                │        后端（FastAPI 单体 backend/）      │
                │  ┌────────┐┌────────┐┌────────┐┌───────┐ │
                │  │ wechat ││modules ││ core   ││ utils │ │
                │  │        ││(三视角)││ 安全/依赖││MinIO等│ │
                │  └────────┘└────────┘└────────┘└───────┘ │
                │  ┌────────┐┌────────┐┌────────┐┌───────┐ │
                │  │ models ││schemas ││services││integr.│ │
                │  └────────┘└────────┘└────────┘└───────┘ │
                └──┬──────────┬──────────┬──────────┬──────┘
                   │          │          │          │
            ┌──────▼───┐ ┌────▼────┐┌────▼────┐┌────▼─────────┐
...
> 说明：后端是**单一 FastAPI 应用**（由内部 projectplatform 的多微服务收敛而来），**AI 能力已拆为独立服务 `ai/`**（默认 8401，后端经 HTTP 调用），依赖 MySQL / Qdrant / Redis 等专业基础设施。**效果优先于部署轻量**——官方实例是门面，知识库与检索采用专业方案。
...
### ② 三大功能模块（`backend/app/modules/{call,tasks,admin}/`）
...
### ③ 知识库模块（`ai/kb/` + `ai/core/retrieval`）——核心护城河
