...
def parse_spec_document(filename: str, raw: bytes) -> str:
    ...
    if ext == ".docx":
        return _clean_markdown(_mammoth_markdown(raw))
    if ext == ".doc":
        return _clean_markdown(_parse_doc(raw))
...
def _mammoth_markdown(raw: bytes) -> str:
    """mammoth 将 .docx 字节转为 markdown。"""
    try:
        import mammoth
...
def _parse_doc(raw: bytes) -> str:
    ...
    try:
        ...
        with open(out_path, "rb") as f:
            docx_raw = f.read()
        return _mammoth_markdown(docx_raw)
